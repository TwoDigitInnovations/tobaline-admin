...
function Queries(props) {
  ...
  const fetchQueries = async (
    ...
    try {
      const res = await Api(
        "post",
        `contactus/getAllContactUs?page=${page}&limit=${limit}`,
        data,
        router,
      );
  ...
  const handleReset = () => {
    const resetParams = { name: "", email: "" };

    setSelectedDate("");
    setSearchParams(resetParams);
    setCurrentPage(1);
    fetchQueries("", 1, 10, resetParams);
  };
  ...
  return (
    <section className="w-full h-[90%] bg-gray-50 p-4 md:p-6 overflow-y-scroll  scrollbar-hide overflow-scroll pb-58">
